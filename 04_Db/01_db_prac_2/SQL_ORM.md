...
2. user 레코드 생성
   ...
   ```python
   # orm
   User.obejcts.create(first_name='서현', last_name='백', age=28, country='경상북도', phone='010-4852-0463',)
   # django.db.utils.IntegrityError: NOT NULL constraint failed: users_user.balance
   ```

   ```sql
   -- sql
   INSERT INTO users_user(first_name, last_name, age, country, phone) VALUES ('서현', '백', 28, '경상북도', '010-4852-0463');
...
2. 나이가 30인 사람의 이름
   ...
   # User.objects.filter(age=30).values('first_name').get('first_name')
   ```
...
3. 나이가 30살 이상인 사람의 인원 수
   ...
   ```
      ...
      ```sql
   -- sql
   SELECT COUNT(*) FROM user_user WHERE age>=30;
      ```
...
4. 나이가 20살 이하인 사람의 인원 수 
   ...
   ```python
   # orm
   User.obects.filter(age__lte=20).count()
   ```
