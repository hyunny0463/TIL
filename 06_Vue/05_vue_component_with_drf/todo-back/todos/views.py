...
def todo_create(request):
    ...
    if serializer.is_valid():
        serializer.save()
        # serializer.data -> Json({ 'id': 1, 'user': 1, 'title': 첫번째 할 일, 'completed': false})
...
def todo_update_delete(request, id):
    ...
    if request.method == 'PUT':
        ...
        todo.delete()
        # 204 -> 해당하는 컨텐츠가 없는 경우
        # (todo를 삭제했기 때문에 해당하는 todo가 존재하지 않음을 알려줌)
...
def user_signup(request):
    ...
    if serializer.is_valid(raise_exception=True):
        # serializer.save()의 return 값은 User 모델의 instance
        user = serializer.save()
        # User model의 instance가 갖고 있는 set_password -> 인자는 raw password가 들어감
